document.addEventListener('DOMContentLoaded', () => { 
    ...
    const translations = {
        de: {
            ...
            export_data: "Daten exportieren", import_data: "Daten importieren",
    ...
    async function completeExtraQuest() {
        ...
        char.mana += quest.manaReward;
        char.gold += quest.goldReward;
        showCustomPopup(`EXTRA-QUEST ERFÜLLT! 📜\n+${quest.manaReward} Mana ✨ | +${quest.goldReward} Gold 💰`);
    ...
    // --- TÄGLICHER CHECK & STRAFSYSTEM (ÜBERARBEITET) ---
    function startDailyCheckTimer() {
        ...
        const checkForPenaltyAndReset = async () => {
            ...
            // 1. Streak- und Inaktivitäts-Strafen (NEUE LOGIK)
            ...
            if (lastDate) {
                ...
                if (diffDays > 1) {
                    ...
                    console.log(`Erkannte Inaktivität: ${missedDays} Tag(e) verpasst.`);
                    
                    setStreakData(0, null); // Streak sofort zurücksetzen
        ...
        console.log(`Nächster täglicher Check in ${Math.round(msUntilMidnight / 60000)} Minuten.`);
        setTimeout(() => {
    ...
    // --- DATEN IMPORT/EXPORT FUNKTIONEN (REPARIERT) ---
    async function exportData() {
        if (!db) {
            showCustomPopup("Datenbank ist nicht bereit. Bitte warten.", 'penalty');
            return;
        }
        ...
        try {
            ...
            // Eine einzige Transaktion für alle Lese-Operationen starten
            ...
            for (const storeName of storeNames) {
                ...
                const promise = new Promise((resolve, reject) => {
                    ...
                    request.onerror = (event) => {
                        reject(new Error(`Fehler beim Exportieren von ${storeName}: ${event.target.error}`));
                    };
                });
                promises.push(promise);
            }

            // Warten, bis alle getAll() Anfragen abgeschlossen sind
            const results = await Promise.all(promises);

            // Das Export-Objekt aus den Ergebnissen zusammenstellen
            for (const result of results) {
                dataToExport[result.name] = result.data;
            }

            // Daten aus dem LocalStorage hinzufügen
            dataToExport.streakData = getStreakData();
            ...
            a.download = `dailyquest-backup-${today}.json`;
            ...
            showCustomPopup("Daten erfolgreich exportiert! 💾");

        } catch (error) {
            console.error("Export fehlgeschlagen:", error);
            showCustomPopup(`Datenexport fehlgeschlagen: ${error.message}`, 'penalty');
        }
    }
    ...
    function importData(event) {
        ...
        const confirmation = confirm("WARNUNG: Dies wird alle Ihre aktuellen Daten überschreiben und die Seite neu laden. Sind Sie sicher?");
        if (!confirmation) {
        ...
        reader.onload = async (e) => {
            try {
                ...
                if (!data.character || !data.settings) {
                    throw new Error("Ungültige oder beschädigte Backup-Datei.");
                }

                if (data.streakData) {
                    // --- BUGFIX START ---
                    let { streak, lastDate } = data.streakData;
                    // Wenn ein positiver Streak ohne lastDate importiert wird,
                    // wird angenommen, dass die letzte Aktivität gestern war, um einen fehlerhaften Reset zu verhindern.
                    if (streak > 0 && !lastDate) {
                        console.warn("Importierte streakData hat einen positiven Streak, aber kein lastDate. Nehme an, die letzte Aktivität war gestern, um den Streak zu erhalten.");
                        lastDate = getYesterdayString();
                    }
                    setStreakData(streak, lastDate);
                    // --- BUGFIX END ---
                } else {
                    // Wenn keine Streak-Daten im Backup sind, aus dem Local Storage entfernen
                ...
                transaction.onerror = (event) => {
                    throw new Error("Fehler beim Schreiben in die Datenbank: " + event.target.error);
                };
