const DQ_SHOP = {
    ...
    renderPage() {
        ...
        charStore.get(1).onsuccess = (e) => {
            ...
            shopStore.getAll().onsuccess = (ev) => {
                ...
                const filteredItems = this.currentShopFilter === 'all' ? allItems : allItems.filter(item => item.type === this.currentShopFilter);
    ...
    buyItem(itemId) {
        ...
        let char; // Variable für den Charakter, um sie in oncomplete zu verwenden
        ...
        .then(([character, item]) => {
            char = character; // Charakter zuweisen
            if (!char || !item) return;
            ...
            char.totalItemsPurchased++; // ACHIEVEMENT-FORTSCHRITT
