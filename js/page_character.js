const DQ_CHARACTER = {
    ...
    createRadarChart(canvas, stats) {
        // BUGFIX: Feste Basisgröße verwenden, um Schrumpfen/Wachsen zu verhindern
        const baseSize = 320;
        ...
        const radius = Math.min(centerX, centerY) * 0.75; // Vergrößert für mehr Fülle
        ...
        const maxStatValue = Math.max(20, ...Object.values(stats));
        ...
        for (let i = 0; i < numAxes; i++) {
            const angle = (i * 2 * Math.PI / numAxes) - (Math.PI / 2);
            const labelRadius = radius * 1.2; // Angepasster Abstand für Emojis
    ...
    async showSellConfirmation(source, location) {
        const db = DQ_DB.db;
        const char = await new Promise(res => db.transaction('character').objectStore('character').get(1).onsuccess = e => res(e.target.result));
