...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadInitialData() {
        ...
        await generateDailyQuestsIfNeeded(); // Aufruf der neuen Funktion
    ...
    // --- NEUE, ROBUSTE QUEST-GENERIERUNG ---
    async function generateDailyQuestsIfNeeded(forceRegenerate = false) {
        const todayStr = DQ_CONFIG.getTodayString();
        const flag = `questsGenerated_${todayStr}`;
        
        // Flag-Check: Wenn heute schon generiert wurde und es nicht erzwungen wird, abbrechen.
        if (localStorage.getItem(flag) && !forceRegenerate) {
            console.log("Quests für heute bereits generiert. Überspringe.");
            return;
        }
        ...
        // Alles in einer Transaktion für atomare Operation
        const quests = await new Promise(res => store.getAll().onsuccess = e => res(e.target.result));
        
        // Alte Quests löschen (egal von welchem Tag, da Dailies nur für heute relevant sind)
        await Promise.all(quests.map(q => new Promise(res => store.delete(q.questId).onsuccess = res)));

        console.log("Alte Quests gelöscht. Generiere neue Quests...");
        ...
        const questCount = (goal === 'restday' || goal === 'sick') ? 3 : 6;
        ...
        newQuests.forEach(questTemplate => {
            ...
            const quest = {
                ...
                completed: false,
                goal: goal // Ziel-Tag für zukünftige Checks
            };
            store.add(quest);
        });
        ...
        return new Promise(resolve => {
            tx.oncomplete = () => {
                localStorage.setItem(flag, 'true'); // Flag setzen nach erfolgreicher Transaktion
            ...
            tx.onerror = (e) => {
                ...
                resolve(); // Trotz Fehler auflösen, um die App nicht zu blockieren
            };
        });
    }
