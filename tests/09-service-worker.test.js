...
function run() {
    ...
    t.ok(cacheMatch && Number(cacheMatch[1]) >= 35, 'Cache-Version fuer Release 2.15.3 erhoeht');
    ...
    t.equal(appVersionMatch && appVersionMatch[1], '2.15.3', 'APP_VERSION ist 2.15.3');
    t.ok(htmlCode.includes('v2.15.3'), 'Settings UI zeigt v2.15.3');
    t.ok(translationsCode.includes('update_point_8'), 'Update-Hinweis fuer grosses wger-Update vorhanden');
    t.ok(databaseCode.includes('dbVersion = 41'), 'IndexedDB-Version fuer Release 2.15.3 bleibt unveraendert');
