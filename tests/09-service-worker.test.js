...
function run() {
    ...
    t.ok(cacheMatch && Number(cacheMatch[1]) >= 28, 'Cache-Version fuer Release 2.13.1 erhoeht');
    ...
    t.equal(appVersionMatch && appVersionMatch[1], '2.13.1', 'APP_VERSION ist 2.13.1');
    t.ok(htmlCode.includes('v2.13.1'), 'Settings UI zeigt v2.13.1');
    t.ok(translationsCode.includes('KI-Trainingsplan erstellt Plaene jetzt wieder zuverlaessig') && translationsCode.includes('AI training plans now create plans reliably again'), 'Update-Hinweis fuer 2.13.1 aktualisiert');
